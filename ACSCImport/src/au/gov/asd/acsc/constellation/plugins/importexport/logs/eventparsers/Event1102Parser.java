...
public class Event1102Parser extends EventTypeParser {

    private static final Logger LOGGER = Logger.getLogger(Event1102Parser.class.getName());

    public Event1102Parser() {
        super("1102 - Security Log Cleared", 2);
    }
    ...
    public void parse(Element input, RecordStore results, PluginParameters parameters) throws IOException {
        ...
        Element eventData = (Element)input.getElementsByTagName("UserData").item(0);
        ...
        NodeList nl = eventData.getElementsByTagName("LogFileCleared");
        for (int i=0;i<nl.getLength();i++)
        {
            Element dp = (Element)nl.item(i);
            for (int j=0;j<dp.getChildNodes().getLength();j++)
            {
                Element dp1 = (Element)dp.getChildNodes().item(j);
                dataPoints.put(dp1.getTagName(), dp1.getTextContent());
            }
        }

        results.add();
        results.set(GraphRecordStoreUtilities.SOURCE + VisualConcept.VertexAttribute.IDENTIFIER, dataPoints.getOrDefault("SubjectUserName",null));
        ...
        results.set(GraphRecordStoreUtilities.TRANSACTION + AnalyticConcept.TransactionAttribute.TYPE, WindowsLogsConcept.TransactionType.LOG_CLEARED);
