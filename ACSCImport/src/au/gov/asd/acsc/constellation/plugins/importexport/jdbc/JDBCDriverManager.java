...
public class JDBCDriverManager {
        ...
        try (final Connection connection = sql.getConnection()) {
            try (final PreparedStatement statement = connection.prepareStatement("SELECT name, path from driver")) {
                try (final ResultSet drivers = statement.executeQuery()) {
                    while (drivers.next()) {
                        JDBCDriver d = new JDBCDriver(drivers.getString("name"), new File(drivers.getString("path")));
    ...
    public JDBCDriver getDriver(String name)
    {
        return __drivers.get(name);
    }
    
    public boolean isDriverUsed(String name)
    {
        try (final Connection connection = sql.getConnection()) {
    ...
    public void removeConnectionsWithDriver(String name)
    {
        try (final Connection connection = sql.getConnection()) {
    ...
    public void addDriver(String name, File jar) 
    {
        try (final Connection connection = sql.getConnection()) {
            File out = new File(driversDir.getAbsolutePath() + File.separator + jar.getName());
    ...
    {
        JDBCDriver d = __drivers.get(name);
        if (d != null)
        {
